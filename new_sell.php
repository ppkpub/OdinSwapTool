...
if(strlen($asset_id)==0){
  \PPkPub\Util::error_exit('./', '无效的ODIN短标识. Invalid Short ODIN.');
}

if(strlen($g_currentUserODIN)==0){
  Header('Location: login.php?backpage=new_sell&asset_id='.urlencode($asset_id));
  exit(-1);
}

if($g_currentUserLevel<2){
  \PPkPub\Util::error_exit('./','该奥丁号帐户只能参与报价！<br>需设置有效身份验证密钥并通过验证后才能发起拍卖。<br>This account only can bid.');
}
...
$tmp_data=\PPkPub\PTTP::getPPkResource(\PPkPub\ODIN::PPK_URI_PREFIX.$asset_id.\PPkPub\ODIN::PPK_URI_RESOURCE_MARK);

if($tmp_data['status_code']!=200){
  \PPkPub\Util::error_exit('./','获取ODIN标识对应资源信息出错. Failed to get ODIN data.');
}
...
<div class="row section">
  <div class="form-group">
    <label for="top_buttons" class="col-sm-5 control-label"><h3><?php echo getLang('发布新拍卖');?> <?php echo getLang('奥丁号');?>[<?php \PPkPub\Util::safeEchoTextToPage( $asset_id );?>]:<?php \PPkPub\Util::safeEchoTextToPage( $full_odin_uri);?></h3></label>
...
<!--资产记录信息
<ul>
<li>资产ID : <?php \PPkPub\Util::safeEchoTextToPage( $asset_id);?></li>
<li>资产名称 : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_asset_info['name']);?></li>
<li>资产代号 : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_asset_info['symbol']);?></li>
<li>资产URI: <?php \PPkPub\Util::safeEchoTextToPage( $full_odin_uri);?></li>
<li>详细说明 : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_asset_info['description']);?></li>
</ul>
-->
<!--ODIN资产信息-->
<ul>
<li><?php echo getLang('管理者比特币地址');?> : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_odin_info['admin']);?></li>
<li><?php echo getLang('附注名称');?> : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_odin_info['title']);?></li>
<li><?php echo getLang('电子邮件');?> : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_odin_info['email']);?></li>

<li><?php echo getLang('配置权限');?> : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_odin_info['auth']);?></li>

<li><?php echo getLang('注册者比特币地址');?> : <?php \PPkPub\Util::safeEchoTextToPage( $tmp_odin_info['register']);?></li>
</ul>
<?php
//检查该标识是否已在拍卖中
...
$rs = mysqli_query($g_dbLink,$sqlstr);
if (!$rs) {
  \PPkPub\Util::error_exit('./', '数据库查询有误，请稍候再试. DB failed,Please retry later!');
}

$row= mysqli_fetch_assoc($rs);
if ($row) {
  \PPkPub\Util::error_exit('./', '指定资产标识有拍卖正在进行中[<a href="sell.php?sell_rec_id='.$row['sell_rec_id'].'">'.$row['sell_rec_id'].'</a>]，不能重复发布. Existed same sell record.');
}
...
<form class="form-horizontal" action="new_sell_confirm.php" method="post">
  ...
  <input type="hidden" name="asset_id" value="<?php \PPkPub\Util::safeEchoTextToPage( $asset_id );?>">
  <input type="hidden" name="from_want_rec_id" value="<?php \PPkPub\Util::safeEchoTextToPage( $from_want_rec_id );?>">
  ...
  <div class="form-group" align="center">
    <div class="col-sm-offset-2 col-sm-10">
      <button class="btn btn-primary btn-lg" type="submit"  onclick="if (window.confirm('<?php echo getLang('注意：拍卖内容一旦提交，将不能更改，确认提交吗？');?>')) {this.innerHTML='Waiting';this.disabled=true;form.submit();  } else {return false;}" ><?php echo getLang('马上发布');?></button>
    </div>
  </div>
