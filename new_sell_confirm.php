...
if(strlen($asset_id)==0){
  \PPkPub\Util::error_exit('./', '无效的奥丁号. Invalid ODIN.');
}
...
//检查所拍卖资产标识有效性
$full_odin_uri=\PPkPub\ODIN::PPK_URI_PREFIX.$asset_id.\PPkPub\ODIN::PPK_URI_RESOURCE_MARK;

$tmp_data=\PPkPub\PTTP::getPPkResource($full_odin_uri);
...
$start_amount=@(0+\PPkPub\Util::safeReqNumStr('start_amount'));
$recommend_names=\PPkPub\Util::safeReqChrStr('recommend_names');
...
if(strlen($from_want_rec_id)>0){
   //给相关求购者发送动态通知
   updateSpecRecords(
        "SELECT want_rec_id,wanter_uri, '".$asset_id."' as sell_odin FROM wants WHERE want_rec_id=".$from_want_rec_id." and status_code=".PPK_ODINSWAP_STATUS_WANT." ;",
        " ",
        'want_rec_id',
        'wanter_uri',
        'sell_odin',
...
<center>
<p><?php echo getLang('对应奥丁号');?>[<?php echo $asset_id;?>]<?php echo getLang('的拍卖信息已发布。');?></p> 
<p><a class="btn btn-success" role="button" href="sell.php?sell_rec_id=<?php echo $new_sell_rec_id;?>"><?php echo getLang('点击这里查看');?></a></p>
</center>
