...
<form class="form-horizontal" action="update_sell_confirm.php" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <span id="start_amount"><?php echo $tmp_sell_record['start_amount']==0 ? getLang('无底价'):trimz($tmp_sell_record['start_amount']);?> (<?php echo getSafeEchoTextToPage(getCoinSymbol($tmp_sell_record['coin_type'])); ?>)</span>
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      ...
      if($tmp_sell_record['status_code']==PPK_ODINSWAP_STATUS_EXPIRED){
          echo '(',getLang('等待拍卖方确拍,'),friendlyTime($tmp_sell_record['end_utc']+PPK_ODINSWAP_OVEETIME_SECONDS),',',getLang('超时将流拍'),')';
      }
      if($tmp_sell_record['status_code']==PPK_ODINSWAP_STATUS_ACCEPT){
          echo '(',getLang('等待报价方支付,'),friendlyTime($tmp_sell_record['accepted_utc']+PPK_ODINSWAP_OVEETIME_SECONDS),',',getLang('超时将流拍'),')';
      }
      ?></span>
    </div>
  </div>   
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      ...
      if($tmp_sell_record['status_code']==PPK_ODINSWAP_STATUS_BID){
        ...
        echo formatTimestampForView($tmp_sell_record['end_utc'],false); 
     }else{
        echo formatTimestampForView($tmp_sell_record['accepted_utc'],false); 
     }
...
?>
    <tr>
        <td><a href="bid.php?bid_rec_id=<?php echo $row['bid_rec_id'];?>"><?php echo trimz($row['bid_amount']) ;?></a> <?php safeEchoTextToPage(getCoinSymbol($row['coin_type'])) ;?><br>
